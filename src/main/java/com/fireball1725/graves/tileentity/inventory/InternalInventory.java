...
public class InternalInventory implements IInventory, Iterable<ItemStack> {
    ...
    public void writeToNBT(NBTTagCompound nbtTagCompound, String name) {
        ...
        nbtTagCompound.setTag(name, tagCompound);
    }
    ...
    public void writeToNBT(NBTTagCompound nbtTagCompound) {
        for (int i = 0; i < this.size; i++) {
            try {
                ...
                nbtTagCompound.setTag("#" + i, tagCompound);
    ...
    public void readFromNBT(NBTTagCompound nbtTagCompound, String name) {
        NBTTagCompound tagCompound = nbtTagCompound.getCompoundTag(name);
        if (tagCompound != null) {
    ...
    public void readFromNBT(NBTTagCompound nbtTagCompound) {
        for (int i = 0; i < this.size; i++) {
            try {
                NBTTagCompound tagCompound = nbtTagCompound.getCompoundTag("#" + i);

                if (tagCompound != null) {
                    this.inventory[i] = ItemStack.loadItemStackFromNBT(nbtTagCompound);
                }
            } catch (Exception ex) {
