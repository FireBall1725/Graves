...
public class SafeBlockReplacer {
    ...
    private static boolean CheckGraveSite(World world, BlockPos blockPos, EnumFacing facing) {
        ...
        if (!CheckBlock(world, Headstone, true))
            placeGrave = false;

        if (!CheckBlock(world, MasterGraveTile, true))
            placeGrave = false;

        if (!CheckBlock(world, SlaveGraveTile1, true))
            placeGrave = false;

        if (!CheckBlock(world, SlaveGraveTile2, false))
            placeGrave = false;

        if (!CheckBlock(world, SlaveGraveTile3, false))
            placeGrave = false;
