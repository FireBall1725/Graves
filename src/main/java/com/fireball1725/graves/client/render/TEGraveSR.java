...
public class TEGraveSR extends TileEntitySpecialRenderer<TileEntityGrave>
{
			...
			else
			{
				if(grave.getProfile() != null)
				{
					...
					if(!state.getBlock().isFullBlock(state.getActualState(getWorld(), grave.getPos().offset(EnumFacing.UP))))
					{
						...
						GlStateManager.pushMatrix();
						if(grave.getProfile() != null && grave.getProfile().getId().toString().equals("4f3a8d1e-33c1-44e7-bce8-e683027c7dac"))
						{
						...
						if(Minecraft.getMinecraft().gameSettings.fancyGraphics)
						{
							drawText(grave.getProfile().getName());
						}
	...
	private void drawText(String text)
	{
		...
		for(EnumFacing facing : EnumFacing.HORIZONTALS)
		{
			...
			GlStateManager.translate(-.375, .55, -.38);
			GlStateManager.scale(.00625, .00625, .00625);
			int strWidth = mc.fontRendererObj.getStringWidth(text);
			mc.fontRendererObj.drawString(text, strWidth / 2, 0, Color.WHITE.hashCode());
			GlStateManager.enableLighting();
			GlStateManager.popMatrix();
		}
		GlStateManager.popMatrix();
